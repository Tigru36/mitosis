...
import { builderContentToMitosisComponent, compileAwayBuilderComponents } from '..';
...
describe('qwik', () => {
  describe('todo', () => {
    ...
    test('Todo.js', async () => {
      const json = parseJsx(todo);
      const fileSet = createFileSet({output: 'mjs', minify: true, jsx: false});
      addComponent(fileSet, json);
      debugOutput(fileSet);
      expect(toObj(fileSet)).toMatchSnapshot();
    });  
  ...
  test('page-with-symbol', async () => {
    const component = builderContentToMitosisComponent(require('./qwik.test.page-with-symbol.json'));
